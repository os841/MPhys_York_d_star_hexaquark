...
//////////////////////////////////////////////////////////////////////////////////////////////////
	...
	//Set Density Ratio and max/min values of hexaquark mass with an appropriate resolution
Double_t density_ratio = 6.0;

Double_t min_hex = 0.0;//2.3*TMath::Power(10,6);//1.945084*TMath::Power(10,12);
Double_t max_hex = 2200;//3.0*TMath::Power(10,6);//1.945094*TMath::Power(10,12);

Double_t res = 1.0;//*TMath::Power(10,2);//2.0*TMath::Power(10,3);//0.1*(max_hex_val / min_hex_val); //depending on range can increase
...
TGaxis::SetExponentOffset(-0.03, -0.07, "X");
...
latex.DrawLatex(0.94, 0.9, "#times10^{-3}");
...
M_N->GetXaxis()->SetNoExponent(kFALSE);
M_N->GetYaxis()->SetNoExponent(kFALSE);
M_N->GetZaxis()->SetNoExponent(kFALSE);

M_N->GetXaxis()->SetMaxDigits(2);
M_N->GetYaxis()->SetMaxDigits(2);
M_N->GetZaxis()->SetMaxDigits(2);

M_N->SetTitle("");
M_N->Draw("colz");
gPad->SetTicks(1,1);           // Ticks on all sides
gPad->SetFixedAspectRatio();   // 1:1 aspect ratio
TLatex *latex2 = new TLatex();
latex2->SetNDC(); // Use normalized device coordinates
latex2->SetTextFont(42);
latex2->SetTextSize(0.04);  // Adjust size as needed
latex2->DrawLatex(0.9, 0.96, "#times10^{-3}"); 
//gStyle->SetPaintTextFormat("1.1e");  // 3 significant figures
